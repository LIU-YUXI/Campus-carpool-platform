...
class AdminLoginForm(Form):
    m_account = StringField('登录名', validators=[DataRequired(), Length(2, 10)])
...
class AddTicketForm(Form):
    ...
    depart_date = DateField(label='出发日期', format='%m/%d/%Y %h:%m:%s')
    depart_time = SelectField('出发时间', choices=date_time)
    arrive_date = DateField(label='到达时期', format='%m/%d/%Y %h:%m:%s ')
    arrive_time = SelectField('到达时间', choices=date_time)
    return_date = DateField(label='返程日期', format='%m/%d/%Y %h:%m:%s')
...
class AddAdminForm(Form):
    m_account = StringField('添加管理员账号', validators=[DataRequired(), Length(2, 10)])
