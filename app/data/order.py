...
class SearchOrder():
    def __init__(self, raw_orders, user_id=None):
        self.orders = []
    ...
    def __parse(self):
        for order in self.raw_orders:
            ...
            if(self.user_id!=None):
                fb= feedback.query.filter_by(o_id=order.id,u_id=self.user_id).first()
                temp_order['content'] = fb.content if fb is not None else '未评论'
            self.orders.append(temp_order)


class SearchOrderWithTotalCM():
    ...
    def __parse(self):
        for order in self.raw_orders:
            ...
            temp_order['arrive_city'] = order.o_daddr
            temp_order['price'] = order.o_price
            fb= feedback.query.filter_by(o_id=order.id).all()
            for i in range(len(fb)):
                temp_order['content'] = fb[i].content
                self.orders.append(temp_order)
